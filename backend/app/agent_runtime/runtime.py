...
from ..providers import ModelProvider, ModelResponse, ToolCall
from .context import compact_llm_messages
from .events import AgentEvent
from .host import AgentRuntimeHost, NullRuntimeHost, TaskCancelled
from .messages import AgentMessage, assistant_tool_content, convert_to_llm
from .tools import Tool, ToolExecutor, ToolResult
from .extensions import ExtensionHook
...
class AgentRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        hook_handlers: list[ExtensionHook] | None = None,
        ...
        self.hook_handlers = list(hook_handlers or [])
        self.host = host or NullRuntimeHost()
        self.context_window = max(2_048, context_window)
        self.max_output_tokens = max(256, max_output_tokens)
        self.context_compaction = context_compaction
        self.context_compaction_trigger_ratio = context_compaction_trigger_ratio
    ...
    async def run(
        ...
        try:
            for result in await self._run_hooks(
                "SessionStart",
            ...
            while turn_index < turn_limit:
                self.host.ensure_not_cancelled(self.task_id)
                ...
                compaction = await compact_llm_messages(
                    ...
                    model=self.model,
                    max_tokens=self.context_window - self.max_output_tokens - 1024,
                    enabled=self.context_compaction,
                    trigger_ratio=self.context_compaction_trigger_ratio,
                )
                ...
                assistant.content = assistant_tool_content(
                    response.content,
                    response.tool_calls,
                ...
                for call in response.tool_calls:
                    ...
                    action = self.host.permission_action(self.permission_mode, tool.capability)
                    if action == "deny":
                        prepared.append((call, tool, ToolResult(
                            call.id, call.name,
                    ...
                    if action == "approve" and self.task_id:
                        approval_id = self.host.create_approval(
                        ...
                        if hook_rejection:
                            self.host.resolve_approval(approval_id, False)
                            approved = False
                        else:
                            approved = await self.host.wait_for_approval(approval_id)
                        self.host.ensure_not_cancelled(self.task_id)
                        if not approved:
                            prepared.append((call, tool, ToolResult(
                ...
                async for call, tool, result in self._execute_prepared(prepared, executor):
                    if tool is not None:
                        for hook_result in await self._run_hooks(
                            "PostToolUse",
                            {
                                "task_id": self.task_id,
                                "tool_name": tool.name,
            ...
            await self._run_hooks("Stop", {"task_id": self.task_id, "reason": "interrupted"}, self.workspace_dir)
            raise
    ...
    async def _stop_events(self, reason: str, error: str | None = None) -> AsyncIterator[AgentEvent]:
        ...
        if error:
            payload["error"] = error
        for result in await self._run_hooks("Stop", payload, self.workspace_dir):
            yield AgentEvent("hook", result)

    async def _run_hooks(self, event: str, payload: dict, cwd: str | None) -> list[dict]:
        results = list(await self.host.run_hooks(event, payload, cwd))
        for handler in self.hook_handlers:
            extension_result = handler(event, payload, cwd)
            if inspect.isawaitable(extension_result):
                extension_result = await extension_result
            results.extend(extension_result or [])
        return results
    ...
    def _consume_mailbox(self, kinds: tuple[str, ...]) -> list[tuple[dict, AgentMessage]]:
        ...
        return [
            ...
            for item in self.host.consume_messages(self.task_id, kinds)
        ]
