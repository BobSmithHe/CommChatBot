...
from .runtime_state import TaskCancelled, runtime_task_manager
from ..infra.database import AgentTask, Conversation, Message, SessionLocal


async def execute_queued_task(task_id: str) -> bool:
    ...
    from ..api.conversation_utils import ensure_conversation_workspace
    from ..services import get_chat_orchestrator
    ...
    if mode == "coding-agent":
        with SessionLocal() as db:
            ...
            workspace = ensure_conversation_workspace(db, conversation)
    ...
    if full_answer:
        ...
        try:
            ...
            provider = (
                get_coding_model_provider() if mode == "coding-agent" else get_chat_model_provider()
            )
            model = (
                settings.coding_model_id if mode == "coding-agent" else settings.chat_model_id
            ) or getattr(provider, "default_model", "") or settings.deepseek_model
            saved_memories, usage = await asyncio.wait_for(
            ...
            if saved_memories:
                runtime_task_manager.append_event(task_id, "memory_updated", {
            ...
            # Memory enrichment is best-effort and must not turn a successful
            # answer into a failed Agent task.
            pass
