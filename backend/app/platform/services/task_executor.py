...
async def execute_queued_task(
    ...
    *,
    ...
    tasks = task_manager or runtime_task_manager
    live = tasks.activate(task_id)
    if not live:
        return False
    with SessionLocal() as db:
        ...
        if not conversation:
            tasks.finish(task_id, "failed", "Conversation no longer exists")
            return False
        try:
    ...
    resume_state = tasks.load_checkpoint(task_id)
    runtime_prompt = (
    ...
    def flush_thinking() -> None:
        ...
        tasks.append_event(task_id, "thinking", payload)
        trace.append({"event": "thinking", "content": payload})
        pending_thinking.clear()
        pending_thinking_chars = 0
        last_thinking_flush = time.monotonic()

    tasks.append_event(task_id, "task", {"task_id": task_id, "status": "running", "resumed": bool(resume_state)})
    try:
        async for event in gateway.stream(
            ...
            if event_type == "answer":
                ...
                if pending_answer_chars >= 256 or now - last_answer_flush >= 0.075:
                    tasks.append_event(task_id, "answer", "".join(pending_answer))
            ...
            if event_type == "intermediate_answer":
                ...
                if intermediate:
                    with SessionLocal() as write_db:
                        ...
                        write_db.commit()
                full_answer = ""
                tasks.append_event(task_id, event_type, content)
                trace = []
                continue
        ...
        flush_thinking()
        tasks.append_event(task_id, "cancelled", str(exc))
        tasks.finish(task_id, "cancelled")
        return True
    except Exception as exc:
        flush_thinking()
        error = f"{type(exc).__name__}: {exc}"
        tasks.append_event(task_id, "error", error)
        tasks.finish(task_id, "failed", error)
        return True
    ...
    if full_answer:
        ...
        try:
            ...
            pass
        tasks.finish(task_id, "completed")
    elif tasks.cancelled(task_id):
        tasks.finish(task_id, "cancelled")
    else:
