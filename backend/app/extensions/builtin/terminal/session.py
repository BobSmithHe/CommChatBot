...
from app.extensions.support.sandbox import (
    ProcessLimitHandle,
...
class WorkspaceTerminalSession:
    ...
    def _spawn(self) -> None:
        if os.name == "nt":
            ...
            node_pty = Path(__file__).resolve().parents[5] / "frontend" / "node_modules" / "node-pty"
        ...
        with self._command_lock:
            self.write(
                (
                    ...
                    f"$commchatCwd=[IO.Path]::GetRelativePath('{escaped_root}', (Get-Location).Path); "
                    f"{marker_expression}; Write-Output ($m + ':' + $commchatExit + ':' + $commchatCwd)\r"
                )
    ...
    def _update_cwd(self, location: str) -> None:
        ...
        if not relative_location.is_absolute():
            self.cwd = (
                "." if ".." in relative_location.parts
                else (relative_location.as_posix() or ".")
            )
            return
        try:
