...
from .support.execution import CodeExecutor
from .builtin.rag import LocalRagStore
from .builtin.workspace import WorkspaceEditor
from ..agent_runtime import Tool


class ProductToolRegistry:
    def __init__(
        self, rag: LocalRagStore, code_executor: CodeExecutor, workspace: WorkspaceEditor,
        *, tavily_api_key: str = "",
    ) -> None:
        self.rag = rag
    ...
    def coding_agent_tools(self, workspace: WorkspaceEditor | None = None) -> list[Tool]:
        """Backward-compatible aggregate; profiles use the smaller extension groups."""
        return [
            *self.workspace_tools(workspace),
            *self.diagnostics_tools(workspace),
            *self.git_tools(workspace),
            *self.terminal_tools(workspace),
        ]

    def workspace_tools(self, workspace: WorkspaceEditor | None = None) -> list[Tool]:
    ...
    def diagnostics_tools(self, workspace: WorkspaceEditor | None = None) -> list[Tool]:
        return [self.get_diagnostics_tool(workspace or self.workspace)]

    def git_tools(self, workspace: WorkspaceEditor | None = None) -> list[Tool]:
        return [self.review_changes_tool(workspace or self.workspace)]

    def terminal_tools(self, workspace: WorkspaceEditor | None = None) -> list[Tool]:
        editor = workspace or self.workspace
        return [self.run_command_tool(editor), self.execute_python_tool(editor)]
    ...
    async def search_web(self, query: str) -> str:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(
                ...
                json={"api_key": self.tavily_api_key, "query": query, "max_results": 5},
            )
