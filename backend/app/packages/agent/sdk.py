...
class AgentSession:
    ...
    def steer(self, content: str) -> bool:
        return bool(
            self.config.task_id
            and runtime_task_manager.enqueue_message(self.config.task_id, "steering", content)
        )

    def follow_up(self, content: str) -> bool:
        return bool(
            self.config.task_id
            and runtime_task_manager.enqueue_message(self.config.task_id, "follow_up", content)
        )

    def cancel(self) -> bool:
        return bool(self.config.task_id and runtime_task_manager.cancel(self.config.task_id))
